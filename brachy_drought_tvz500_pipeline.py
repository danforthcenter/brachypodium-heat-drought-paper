...
def main():
    ...
    s_thresh = pcv.threshold.binary(gray_img=s, threshold=110, max_value=255, object_type='light')

    b = pcv.rgb2gray_lab(rgb_img=img_wb, channel='b')

    b_thresh = pcv.threshold.binary(gray_img=b, threshold=140, max_value=255, object_type='light')
    ...
    roi_contour, roi_hierarchy= pcv.roi.rectangle(img=img_wb, x=500, y=300, h=1500, w=1500)
    ...
    masked1, binary, contours, hierarchy = pcv.rectangle_mask(img=masked, p1=(1460,895), p2=(1520,980), color="black")
    
    masked2, binary, contours, hierarchy = pcv.rectangle_mask(img=masked1, p1=(0,0), p2=(350,2200), color="black")
    
    masked3, binary, contours, hierarchy = pcv.rectangle_mask(img=masked2, p1=(2200,0), p2=(2500,2200), color="black")
    
    nbmask = pcv.naive_bayes_classifier(masked3, "brachy_naive_bayes_pdfs.txt")
    ...
    if args.coresult is not None:
        ...
        newmask = pcv.crop_position_mask(img=nir, mask=nmask, x=10, y=10, v_pos="bottom", h_pos="right")
