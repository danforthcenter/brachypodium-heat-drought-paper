...
ranked_drought <- read.csv("./accessions_ranked_drought.csv")
ranked_heat <- read.csv("./accessions_ranked_heat.csv")
ranked_heatdrought <- read.csv("./accessions_ranked_heatdrought.csv")

setwd("/Users/eludwig/Google Drive/My Drive/brachy-gapit/z_test")
ranked_drought <- select(ranked_drought, "Accession", "mean_z")
ranked_heat <- select(ranked_heat, "Accession", "mean_z")
ranked_heatdrought <- select(ranked_heatdrought, "Accession", "mean_z")

colnames(ranked_drought)[2] <- "z_drought"
colnames(ranked_heat)[2] <- "z_heat"
colnames(ranked_heatdrought)[2] <- "z_heatdrought"
ranked_stresses <- inner_join(ranked_drought, ranked_heat, by="Accession")
ranked_stresses <- inner_join(ranked_stresses, ranked_heatdrought, by="Accession")

write.table(ranked_stresses, "./ranked_stresses_z_GAPIT.txt")
ranked_stresses <- read.table("./ranked_stresses_z_GAPIT.txt", header = TRUE)


setwd("/Users/eludwig/Google Drive/My Drive/brachy-gapit/z_combined_test")
ranked_stresses <- read.table("./combined_z.txt", header = TRUE)
ranked_stresses <- select(ranked_stresses, Accession, Means)

in_gwas <- unique(ranked_stresses$Accession) 
...
hist(ranked_stresses$Means)
mean(ranked_stresses$Means)
range(ranked_stresses$Means)
sd(ranked_stresses$Means)
which(is.na(ranked_stresses$Means))

#check phenotype file
hist(ranked_stresses$z_drought)
mean(ranked_stresses$z_drought)
range(ranked_stresses$z_drought)
sd(ranked_stresses$z_drought)
which(is.na(ranked_stresses$z_drought))

hist(ranked_stresses$z_heat)
mean(ranked_stresses$z_heat)
range(ranked_stresses$z_heat)
sd(ranked_stresses$z_heat)
which(is.na(ranked_stresses$z_heat))

hist(ranked_stresses$z_heatdrought)
mean(ranked_stresses$z_heatdrought)
range(ranked_stresses$z_heatdrought)
sd(ranked_stresses$z_heatdrought)
which(is.na(ranked_stresses$z_heatdrought))
...
source("https://zzlab.net/GAPIT/previous/gapit_20220411.txt")
...
# Didn't end up using them in final run because best fit model didn't include
