...
def main():
    ...
    s_thresh = pcv.threshold.binary(gray_img=s, threshold=45, max_value=255, object_type='light')
    ...
    roi_contour, roi_hierarchy= pcv.roi.rectangle(img=img_wb, x=700, y=500, h=670, w=900)
    ...
    height_img = pcv.analyze_bound_horizontal(img=img_wb, obj=obj, mask=mask, line_position=1210)
    ...
    masked1, binary, contours, hierarchy = pcv.rectangle_mask(img=masked, p1=(0,1520), p2=(2500,2055), color="black")
    
    nbmask = pcv.naive_bayes_classifier(masked1, "brachy_naive_bayes_pdfs.txt")
    ...
    if args.coresult is not None:
        ...
        newmask = pcv.crop_position_mask(img=nir, mask=nmask, x=68, y=2, v_pos="top", h_pos="left")
